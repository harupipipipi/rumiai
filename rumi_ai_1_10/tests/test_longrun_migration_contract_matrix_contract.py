...
MATRIX_PATH = QUALITY_PACK_DIR / "longrun_migration_contract_matrix.yaml"
SCENARIO_FILES = sorted(QUALITY_PACK_DIR.glob("manual_regression_scenarios*.yaml"))

REQUIRED_SECTIONS = {
    "hmac_migration_contracts",
    "store_migration_contracts",
    "startup_failsoft_contracts",
    "compatibility_cli_contracts",
}

REQUIRED_APP_SIGNATURES = [
    "migrate-hmac",
    "_run_hmac_migration",
    "_check_permissive_production_guard",
    "validate_host_execution",
    "Pack API server running on http://",
]

REQUIRED_MIGRATION_SIGNATURES = [
...
REQUIRED_HMAC_SIGNATURES = {
    "core_runtime/capability_trust_store.py": [
        "def hmac_status(self) -> str:",
        "def migrate_hmac_signature(self) -> str:",
        '"missing"',
        '"invalid"',
    ],
    "core_runtime/store_sharing_manager.py": [
        "def hmac_status(self) -> str:",
        "def migrate_hmac_signature(self) -> str:",
        '"missing"',
        '"invalid"',
    ],
    "core_runtime/capability_installer.py": [
...
def _assert_contract_shape(section: dict) -> None:
    ...
    assert len(section["required_files"]) >= 2
    assert len(section["required_behaviors"]) >= 4
    assert len(section["key_manual_scenarios"]) >= 6
    assert len(section["automated_tests"]) >= 2


def test_longrun_migration_contract_matrix_exists_and_sections_present():
...
def test_longrun_migration_contract_matrix_section_shapes():
    matrix = _load_matrix()
    for section_name in REQUIRED_SECTIONS:
        _assert_contract_shape(matrix[section_name])


def test_longrun_migration_contract_matrix_references_existing_files_and_tests():
    matrix = _load_matrix()
    all_files: list[str] = []
    all_tests: list[str] = []
    for section_name in REQUIRED_SECTIONS:
...
def test_longrun_migration_contract_matrix_manual_scenarios_exist():
...
def test_longrun_migration_contract_signatures_are_present():
    app_text = (PACKAGE_ROOT / "app.py").read_text(encoding="utf-8")
    for signature in REQUIRED_APP_SIGNATURES:
        assert signature in app_text

    migration_text = (PACKAGE_ROOT / "core_runtime" / "store_migration.py").read_text(
        encoding="utf-8"
    )
    for signature in REQUIRED_MIGRATION_SIGNATURES:
        assert signature in migration_text

    for rel_path, signatures in REQUIRED_HMAC_SIGNATURES.items():
...
def test_longrun_migration_contract_quality_scripts_include_test():
    quality_script = (
    ...
    assert "tests/test_longrun_migration_contract_matrix_contract.py" in quality_script
    assert "tests/test_longrun_migration_contract_matrix_contract.py" in debug_script
