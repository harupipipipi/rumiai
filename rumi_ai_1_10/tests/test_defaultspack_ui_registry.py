...
class TestDefaultspackUiRegistry(unittest.TestCase):
    ...
    def test_pack_shell_renderer_modules_are_namespaced_and_digest_bound(self):
        from domain.frontend.registry import FrontendRegistry
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            pack_root = ecosystem_root / "defaultspack"
            memo_pack = ecosystem_root / "rumi_memo_pack"
            (pack_root / "ecosystem.json").parent.mkdir(parents=True, exist_ok=True)
            (pack_root / "ecosystem.json").write_text(json.dumps({"pack_id": "defaultspack"}), encoding="utf-8")
            (memo_pack / "frontend_extensions").mkdir(parents=True, exist_ok=True)
            (memo_pack / "static" / "renderers").mkdir(parents=True, exist_ok=True)
            (memo_pack / "ecosystem.json").write_text(json.dumps({"pack_id": "rumi_memo_pack"}), encoding="utf-8")
            (memo_pack / "static" / "renderers" / "memo.js").write_text("export default function Memo(){}", encoding="utf-8")
            (memo_pack / "frontend_extensions" / "memo.ui.json").write_text(
                json.dumps(
                    {
                        "shell_renderers": [
                            {
                                ...
                                "component": "MemoNav",
                                "regions": ["memo_nav"],
                                "module": "/static/renderers/memo.js",
                                "export": "default",
                                "trust": "local",
                            }
                        ],
            ...
            with patch("domain.frontend.registry.AIClient") as mock_client, patch.object(
                FrontendRegistry,
                "_model_options",
                return_value=[],
            ), patch.object(
        ...
        renderer = {item["id"]: item for item in catalog["shell"]["renderers"]}["memo_nav"]
        self.assertEqual(renderer["source_pack_id"], "rumi_memo_pack")
        self.assertEqual(renderer["module_pack_id"], "rumi_memo_pack")
        self.assertEqual(renderer["module"], "/static/packs/rumi_memo_pack/renderers/memo.js")
        self.assertTrue(renderer["integrity"].startswith("sha256-"))
        self.assertNotIn("shell_renderer_untrusted_module", {item["code"] for item in catalog["diagnostics"]})
    ...
    def test_static_renderer_serving_can_read_selected_pack_static_renderers(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                renderer = server._handle_static_file({}, {"path": "packs/rumi_memo_pack/renderers/memo.js"})
                legacy_renderer = server._handle_static_file({}, {"path": "renderers/memo.js"})
        ...
        self.assertEqual(legacy_renderer["status"], "error")
        self.assertEqual(hidden["status"], "error")

    def test_static_renderer_serving_does_not_scan_or_shadow_pack_namespaces(self):
        from transport.http import DefaultsHttpServer
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            defaultspack_root = ecosystem_root / "defaultspack"
            memo_pack_root = ecosystem_root / "rumi_memo_pack"
            shadow_pack_root = ecosystem_root / "aaa_shadow_pack"
            (defaultspack_root / "ui").mkdir(parents=True)
            (memo_pack_root / "static" / "renderers").mkdir(parents=True)
            (shadow_pack_root / "static" / "renderers").mkdir(parents=True)
            (defaultspack_root / "ecosystem.json").write_text("{}", encoding="utf-8")
            (memo_pack_root / "ecosystem.json").write_text("{}", encoding="utf-8")
            (shadow_pack_root / "ecosystem.json").write_text("{}", encoding="utf-8")
            (memo_pack_root / "static" / "renderers" / "memo.js").write_text(
                "export default 'memo';",
                encoding="utf-8",
            )
            (shadow_pack_root / "static" / "renderers" / "memo.js").write_text(
            ...
            try:
                ...
                http_module.__file__ = str(defaultspack_root / "transport" / "http.py")
                renderer = server._handle_static_file({}, {"path": "packs/rumi_memo_pack/renderers/memo.js"})
                shadow = server._handle_static_file({}, {"path": "packs/aaa_shadow_pack/renderers/memo.js"})
            finally:
                http_module.__file__ = original_file

        self.assertIn("memo", renderer["body"])
