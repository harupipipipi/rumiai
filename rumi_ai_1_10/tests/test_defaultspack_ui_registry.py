...
class TestDefaultspackUiRegistry(unittest.TestCase):
    ...
    def test_dynamic_settings_hydration_reuses_model_and_provider_status(self):
        from domain.frontend.registry import FrontendRegistry
        ...
        sections = [
            {
                ...
                "fields": [
                    {"id": "preferred_model"},
                    {"id": "secondary_model", "type": "model_select"},
                    {"id": "model_api_routes"},
                ],
            },
            {"id": "apis", "fields": [{"id": "api_keys"}]},
        ]
        profiles = [
            {
        ...
        with patch(
            "ecosystem.defaultspack.backend.ai_client.provider_catalog.list_profile_catalog",
            return_value=profiles,
        ) as list_profiles, patch(
            "domain.frontend.registry.provider_key_status",
            return_value=[{"provider_id": "stub", "configured": True}],
        ) as key_status:
            hydrated = registry._hydrate_dynamic_settings_fields(sections)

        models_fields = hydrated[0]["fields"]
        apis_fields = hydrated[1]["fields"]
        self.assertEqual(list_profiles.call_count, 1)
        self.assertEqual(key_status.call_count, 1)
        self.assertTrue(models_fields[0]["options"])
        self.assertTrue(models_fields[1]["options"])
        self.assertEqual(models_fields[2]["api_keys"][0]["provider_id"], "stub")
        self.assertEqual(apis_fields[0]["api_keys"][0]["provider_id"], "stub")

    def test_frontend_registry_reuses_provider_models_for_model_panels(self):
        from domain.frontend.registry import FrontendRegistry

        registry = FrontendRegistry(pack_root=DEFAULTSPACK_ROOT)
        items = [
            {"id": "one", "panel": {"kind": "models"}},
            {"id": "two", "panel": {"kind": "models"}},
        ]

        with patch("domain.frontend.registry.AIClient") as mock_client:
            mock_client.return_value.list_models.return_value = [{"id": "stub/default"}]
            hydrated = registry._hydrate_sidebar_items(items)

        self.assertEqual(mock_client.return_value.list_models.call_count, 1)
        self.assertEqual(hydrated[0]["panel"]["models"], [{"id": "stub/default"}])
        self.assertEqual(hydrated[1]["panel"]["models"], [{"id": "stub/default"}])

    def test_frontend_registry_reuses_oauth_statuses_within_request(self):
        from domain.frontend.registry import FrontendRegistry
        ...
        with patch(
            ...
            return_value={"google": {"provider_id": "google", "connected": False}},
        ) as statuses:
            first = registry._provider_oauth_statuses()
            second = registry._provider_oauth_statuses()

        self.assertEqual(statuses.call_count, 1)
