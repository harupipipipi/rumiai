...
class TestCapabilityNodeHandlers(unittest.TestCase):
    def test_get_nodes_returns_registry_payload(self):
        handler = _FakeHandler()
        registry = SimpleNamespace(
            diagnostics=[{"level": "warning", "code": "test"}],
            to_public_list=lambda: [{"node_id": "rumi.start"}],
        )
        with patch.object(
            ControlPanelHandlersMixin,
            "_panel_capability_node_registry",
            return_value=registry,
        ):
            result = handler._panel_get_nodes()

        self.assertEqual(result["count"], 1)
        self.assertEqual(result["nodes"][0]["node_id"], "rumi.start")
        self.assertEqual(result["diagnostics"][0]["code"], "test")

    def test_get_profile_nodes_returns_profile_scoped_state(self):
                ...
                "nodes": [
                    {
                        "node_id": "pack.agent",
                        "ports": [],
                        "requirements": {"required_settings": ["credential_ref"]},
                    }
                ],
            },
            pack_id="pack",
        ...
        profile = CapabilityProfileDefinition(
            profile_id="pack.coding",
            enabled_nodes=["pack.agent"],
            node_settings={"pack.agent": {"credential_ref": "secret://agent"}},
        )
        profile_loader = SimpleNamespace(
            diagnostics=[],
            get_profile=lambda profile_id: profile if profile_id == "pack.coding" else None,
        )
        node_registry = SimpleNamespace(
            diagnostics=[],
            nodes={"pack.agent": node},
            load_all_nodes=lambda: {"pack.agent": node},
        )

        handler = _FakeHandler()
        with patch.object(
            ControlPanelHandlersMixin,
            "_panel_capability_profile_loader",
            return_value=profile_loader,
        ), patch.object(
            ControlPanelHandlersMixin,
            "_panel_capability_node_registry",
            return_value=node_registry,
        ):
            result = handler._panel_get_profile_nodes("pack.coding")

        self.assertEqual(result["profile"]["profile_id"], "pack.coding")
        self.assertEqual(result["count"], 1)
        self.assertEqual(result["nodes"][0]["node_id"], "pack.agent")
