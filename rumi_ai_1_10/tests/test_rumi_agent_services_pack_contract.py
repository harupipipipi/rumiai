from __future__ import annotations



import json
import re
from pathlib import Path

import yaml
import pytest

pytestmark = pytest.mark.contract


ROOT = Path(__file__).resolve().parent.parent
PACK_DIR = ROOT / "ecosystem" / "rumi_agent_services_pack"
SETUP_PACK_JSON = ROOT / "ecosystem" / "setup_pack" / "rumi_agent_services_pack" / "pack.json"


def _yaml_files() -> list[Path]:
    return sorted(PACK_DIR.glob("**/*.yaml"))


def test_agent_services_pack_required_docs_exist():
    required = [
        PACK_DIR / "README.md",
        PACK_DIR / "docs" / "README.md",
        PACK_DIR / "docs" / "architecture.md",
        PACK_DIR / "docs" / "interfaces.md",
        PACK_DIR / "docs" / "operations.md",
    ]

    missing = [str(path.relative_to(ROOT)) for path in required if not path.is_file()]

    assert missing == []


def test_agent_services_pack_json_and_setup_dependencies():
    ecosystem = json.loads((PACK_DIR / "ecosystem.json").read_text(encoding="utf-8"))
    setup = json.loads(SETUP_PACK_JSON.read_text(encoding="utf-8"))

    assert ecosystem["pack_id"] == "rumi_agent_services_pack"
    assert ecosystem["runtime"]["type"] == "declarative_pack"
    assert setup["target_pack_id"] == "rumi_agent_services_pack"
    assert setup["recommended"] is False
    assert setup["conflicts_with"] == []
    assert setup["overlap_policy"]["defaultspack_core"] == "do_not_override"
    assert setup["defaultspack_promotion"]["eligible"] is False

    dependencies = {item["pack_id"]: item.get("version") for item in setup["depends_on"]}

    assert dependencies == {
        "defaultspack": ">=2.0.0",
        "rumi_default_tools_pack": ">=1.0.0",
        "rumi_local_agent_pack": ">=1.0.0",
    }
    assert setup["conflict_notes"]
    assert setup["security_notes"]


def test_agent_services_pack_yaml_files_parse_and_have_ids():
    yaml_files = _yaml_files()

    assert yaml_files

    for path in yaml_files:
        data = yaml.safe_load(path.read_text(encoding="utf-8"))
        assert isinstance(data, dict), path
        assert any(key.endswith("_id") or key == "id" for key in data), path


def test_agent_services_profiles_are_local_first_and_network_denied():
    for path in sorted((PACK_DIR / "profiles").glob("*.profile.yaml")):
        data = yaml.safe_load(path.read_text(encoding="utf-8"))
        policy = data.get("policy", {})

        assert data["profile_id"].startswith("rumi_agent_services.")
        assert policy.get("local_first") is True
        assert policy.get("network_default") == "deny"


def test_agent_services_pack_contains_no_secret_like_literals():
    secret_patterns = [
        re.compile(pattern, re.IGNORECASE)
        for pattern in [
            r"api[_-]?key\s*[:=]\s*['\"]?[A-Za-z0-9_-]{12,}",
            r"bearer\s+[A-Za-z0-9._-]{16,}",
            r"sk-[A-Za-z0-9]{16,}",
            r"password\s*[:=]\s*['\"][^'\"]+['\"]",
            r"oauth[_-]?client[_-]?secret\s*[:=]",
        ]
    ]

    checked_files = [
        path
        for path in PACK_DIR.glob("**/*")
        if path.is_file() and path.suffix in {".json", ".yaml", ".md"}
    ]
    checked_files.append(SETUP_PACK_JSON)

    offenders: list[str] = []
    for path in checked_files:
        text = path.read_text(encoding="utf-8")
        if any(pattern.search(text) for pattern in secret_patterns):
            offenders.append(str(path.relative_to(ROOT)))

    assert offenders == []
