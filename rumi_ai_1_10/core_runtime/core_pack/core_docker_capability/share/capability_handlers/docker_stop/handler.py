"""
docker_stop handler - Stop a running Docker container
via DockerCapabilityHandler.

This handler is the entry point for docker.stop capability requests.
It delegates to DockerCapabilityHandler (provided by W22-D) which
validates permissions and stops the container.
"""
...
def execute(context: dict, args: dict) -> dict:
    ...
    args : dict
        Arguments matching input_schema (container_name, timeout).

    Returns
    -------
    dict
        Result with stopped (bool), container_name,
