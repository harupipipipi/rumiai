...
def _computer_result_envelope(
    ...
    *,
    ...
    if exact_selection_required:
        ...
        selection = safe_window_selection_facts(result_value, requested_app=requested_app)
        valid = bool(
    ...
    if action == "computer.probe_text_control":
        ...
        safe_result.update({"executed": False, "probe_completed": False, "is_error": True})
    ...
    if action == "computer.key" and not _approval_pending(result):
        delivery = safe_computer_delivery_facts(result)
        if delivery.get("delivered") is True and delivery.get("completion_verified") is not True:
            return {
                "ok": False,
...
def _approval_pending(result: object) -> bool:
    return isinstance(result, dict) and bool(result.get("approval_required") or result.get("requires_approval"))
...
def _verified_screenshot_facts(result: object, artifact_root: Path | None) -> dict[str, object]:
    ...
    facts["capture_driver"] = capture_driver if capture_driver in _SCREENSHOT_CAPTURE_DRIVERS else "none"
...
def _verified_artifact(raw_path: object, artifact_root: Path | None) -> tuple[bool, bool, bool]:
    ...
    try:
        ...
        regular_nonempty = stat.S_ISREG(details.st_mode) and not candidate.is_symlink() and details.st_size > 0
