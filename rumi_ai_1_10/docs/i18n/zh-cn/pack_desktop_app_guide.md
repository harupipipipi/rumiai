...
|领域 |类型 |必填 |描述 |
|-----------|-----|------|------|
| `command`|字符串| ✅ |启动命令。作为 pack-shell 的 `--command` 参数传递给应用程序 |
| `working_dir`|字符串| — |应用程序的工作目录。如果为空字符串，将使用 Pack 目录 |
| `env`|字典 | — |要传递给应用程序的其他环境变量。 `RUMI_TOKEN`、`RUMI_PORT`、`RUMI_PACK_ID` 不是必需的，因为 pack-shell 会自动配置它们 |
| `capabilities`|列表 | — |请求的功能列表 |
| `window`|字典 | — |窗口设置。使用`title`（字符串）指定应用程序名称，并使用`width`/`height`（整数）指定大小。也用于快捷方式名称 |
| `platforms`|列表 | — |支持的平台。 `"darwin"`、`"win32"`、`"linux"`的组合|
...
|论点|类型 |必填 |默认 |描述 |
|------|-----|------|-----------|------|
| `<PACK_ID>`|位置论证 | ✅ | — |要启动的包的 ID |
| `--command`|字符串| ✅ | — |要执行的命令（例如`"python app.py"`）|
| `--api-token`|字符串| ✅ |环境变量`RUMI_API_TOKEN` |内核 API 身份验证令牌 |
| `--port`| u16 | 16 — | `8765` |内核API端口号|
| `--kernel-cmd`|字符串| — | `"python -m rumi_ai"` |内核未启动时启动的命令 |
| `--timeout`| u64 | 64 — | `60` |等待内核启动超时时间（秒）|
| `--working-dir`|字符串| — |无 |应用程序工作目录 |
...
|变量|描述 |
|------|------|
| `RUMI_API_TOKEN`|更换`--api-token`。 CLI 参数优先 |
...
|-----------|-----|------|
| `token`|字符串|短期访问令牌 |
| `port`|整数 |内核API端口号（默认：8765）|
| `expires_in`|整数 |令牌过期时间（秒；默认值：3600）|
...
|方法|描述 |
|--------|------|
| `register_app(pack_id, desktop_app_config, pack_dir)`|注册 Pack 桌面应用程序并生成特定于平台的快捷方式 |
| `unregister_app(pack_id)`|取消订阅并删除快捷方式 |
| `launch_app(pack_id)`|启动已注册的应用程序 |
| `stop_app(pack_id)`|使用 SIGTERM 停止正在运行的应用程序 |
| `list_registered_apps()`|返回已注册应用程序列表 |
...
|平台|格式|地点 |
|---------------|------|--------|
| macOS（`darwin`）| `.app` 捆绑包（Info.plist + 启动脚本）| `~/Applications/<AppName>.app` |
| Windows (`win32`) | `.lnk` 快捷方式（使用 PowerShell 生成）| `user_data/apps/<AppName>.lnk` |
| Linux | `.desktop`文件| `~/.local/share/applications/rumi-<AppName>.desktop` |
