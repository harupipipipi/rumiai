...
| 필드 | 유형 | 필수 | 설명 |
|-----------|-----|------|------|
| `command` | 문자열 | ✅ | 명령을 시작합니다. pack-shell의 `--command` 인수로 앱에 전달됨 |
| `working_dir` | 문자열 | — | 앱의 작업 디렉터리입니다. 빈 문자열인 경우 Pack 디렉터리가 사용됩니다. |
| `env` | 사전 | — | 앱에 전달할 추가 환경 변수입니다. `RUMI_TOKEN`, `RUMI_PORT`, `RUMI_PACK_ID`는 팩 쉘이 자동으로 구성하므로 필요하지 않습니다 |
| `capabilities` | 목록 | — | 요청된 기능 목록 |
| `window` | 사전 | — | 창 설정. `title`(문자열)으로 앱 이름을 지정하고 `width`/`height`(int)로 크기를 지정합니다. 바로가기 이름에도 사용됨 |
| `platforms` | 목록 | — | 지원되는 플랫폼. `"darwin"`, `"win32"`, `"linux"`의 조합 |
...
| 액션 | 설명 |
|--------|------|
| `launch` | 앱 시작 및 토큰 발행(기본값) |
| `stop` | 앱 실행 중지 |
| `status` | 앱의 실행 상태를 반환합니다 |
...
| 인수 | 유형 | 필수 | 기본값 | 설명 |
|------|-----|------|-----------|------|
| `<PACK_ID>` | 위치 인수 | ✅ | — | 출시할 팩의 ID |
| `--command` | 문자열 | ✅ | — | 실행할 명령(예: `"python app.py"`) |
| `--api-token` | 문자열 | ✅ | 환경 변수 `RUMI_API_TOKEN` | 커널 API 인증 토큰 |
| `--port` | u16 | — | `8765` | 커널 API 포트 번호 |
| `--kernel-cmd` | 문자열 | — | `"python -m rumi_ai"` | 커널이 시작되지 않은 경우 시작 명령 |
| `--timeout` | u64 | — | `60` | 커널 시작 대기 시간 초과(초) |
| `--working-dir` | 문자열 | — | 없음 | 앱 작업 디렉토리 |
...
| 변수 | 설명 |
|------|------|
| `RUMI_API_TOKEN` | `--api-token` 대체. CLI 인수가 우선 적용됩니다 |
...
|-----------|-----|------|
| `token` | 문자열 | 단기 액세스 토큰 |
| `port` | 정수 | 커널 API 포트 번호(기본값: 8765) |
| `expires_in` | 정수 | 토큰 만료 시간(초, 기본값: 3600) |
...
| 방법 | 설명 |
|--------|------|
| `register_app(pack_id, desktop_app_config, pack_dir)` | Pack 데스크톱 앱 등록 및 플랫폼별 바로가기 생성 |
| `unregister_app(pack_id)` | 바로가기 구독 취소 및 삭제 |
| `launch_app(pack_id)` | 등록된 앱 시작 |
| `stop_app(pack_id)` | SIGTERM |
| `list_registered_apps()` | 등록된 앱 목록 반환 |
...
| 플랫폼 | 형식 | 위치 |
|---------------|------|--------|
| macOS(`darwin`) | `.app` 번들(Info.plist + 실행 스크립트) | `~/Applications/<AppName>.app` |
| 윈도우(`win32`) | `.lnk` 바로가기(PowerShell로 생성됨) | `user_data/apps/<AppName>.lnk` |
| 리눅스 | `.desktop` 파일 | `~/.local/share/applications/rumi-<AppName>.desktop` |
