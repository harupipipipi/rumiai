...
| frontend/UI | `tests/test_frontend_ux_contract_matrix_contract.py`, `tests/test_viewer_release_contract_matrix_contract.py`, `tests/test_ui_viewer_recovery_contract_matrix_contract.py`, `frontend_ux_contract_matrix.yaml`, `viewer_release_contract_matrix.yaml`, `ui_viewer_recovery_contract_matrix.yaml`, `frontend lint/build` 実行 | panel と viewer の状態遷移・DOM契約・導線・復旧導線・ビルド破綻の回帰検知 |
| security/permission | `tests/test_security_guards.py`, `tests/test_capability_*` | 最小権限・承認・ガード・能力境界の破壊を検知 |
| failure-path | `tests/test_wave20d_rate_limit.py`, `tests/test_security_guards.py`, `tests/test_runtime_boundary_contract_matrix_contract.py` | 外部失敗・ガード失敗・runtime境界失敗時の挙動を検証 |
