...
class AIClientLoader:
    ...
    def __init__(self):
        """ローダーを初期化"""
        self.ai_client_dir = Path(__file__).parent
        self.loaded_clients = {}
        self.model_profiles = {}
    ...
    def _load_dependency_manager(self):
        ...
        # ai_client_dependency_manager.py を探す（リネーム後のファイル名）
        dependency_manager_path = self.ai_client_dir / "ai_client_dependency_manager.py"
        
        # フォールバック: 旧ファイル名も確認
        if not dependency_manager_path.exists():
            dependency_manager_path = self.ai_client_dir / "dependency_manager.py"
        
        if not dependency_manager_path.exists():
            print(f"警告: dependency_manager が見つかりません: {self.ai_client_dir}")
            return None
