...
class BrowserComputerController:
    ...
    def _computer_read_action(self, action: str, payload: dict[str, Any], *, yolo_mode: bool) -> dict[str, Any]:
        ...
        swift_payload, target_error = self._swift_action_payload_or_target_error(action, payload)
        if target_error is not None:
            return target_error
        swift_result = self._darwin_swift_optional_action_result(action, swift_payload)
    ...
    def _computer_click_text(self, payload: dict[str, Any], *, yolo_mode: bool) -> dict[str, Any]:
        ...
        swift_payload, target_error = self._swift_action_payload_or_target_error("computer.click_text", payload)
        if target_error is not None:
            return target_error
        swift_result = self._darwin_swift_optional_action_result("computer.click_text", swift_payload)
        if swift_result is not None:
    ...
    def _swift_action_payload_or_target_error(
        self,
        action: str,
        ...
        if target is None and self._has_window_filter(payload) and not self._explicit_desktop_target(payload):
            return dict(payload), self._target_mismatch_result(action, payload)
        return self._swift_payload_with_target(payload, target), None

    @classmethod
    def _swift_payload_with_target(
        cls,
        ...
        if title is not None:
            args.setdefault("window_title", title)
        for rect_key in ("capture_rect", "content_rect"):
            rect = cls._normalize_rect(target.get(rect_key))
            if rect:
                args.setdefault(rect_key, rect)
        return args

    @staticmethod
    def _explicit_desktop_target(payload: dict[str, Any] | None = None) -> bool:
        payload = payload or {}
        target = str(payload.get("target") or payload.get("capture_target") or "").strip().lower()
        return target in {"primary_display", "all_displays", "screen", "display", "desktop"}

    def _target_mismatch_result(self, action: str, payload: dict[str, Any]) -> dict[str, Any]:
        result: dict[str, Any] = {
            "action": action,
            "platform": platform.system(),
            "supported": False,
