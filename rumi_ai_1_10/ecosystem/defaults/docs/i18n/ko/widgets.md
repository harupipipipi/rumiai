...
|---|---|---|
| `Text` | 텍스트 표시 | `text` |
| `CodeBlock` | 소스 코드 보기 | `language`, `content`, `filename`, `line_start` |
| `Diff` | 차이 표시 | `old_content`, `new_content`, `filename` |
| `Image` | 이미지 표시 | `src`, `alt`, `width`, `height` |
| `Screenshot` | 스크린샷 표시 | `src`, `url`, `title` |
| `Progress` | 진행상황 표시 | `label`, `current`, `total`, `state` |
| `Terminal` | 단자 출력 표시 | `command`, `output`, `exit_code` |
| `Table` | 테이블 표시 | `headers`, `rows` |
| `Chart` | 그래프 표시 | `chart_type`, `labels`, `data` |
| `FileTree` | 파일 트리 표시 | `tree` |
| `Markdown` | 마크다운 렌더링 | `content` |
| `Audio` | 오디오 재생 | `src`, `duration` |
| `Video` | 비디오 재생 | `src`, `duration` |
| `Map` | 지도 표시 | `lat`, `lng`, `zoom` |

### controls.py(6가지 유형의 컨트롤)

| 수업 | 사용법 | 주요 매개변수 |
|---|---|---|
| `Input` | 텍스트 입력 | `placeholder`, `value`, `multiline` |
| `Button` | 버튼 | `label`, `action`, `variant` |
| `Select` | 선택 | `options`, `value`, `multiple` |
| `Toggle` | 토글 스위치 | `label`, `value` |
| `Slider` | 슬라이더 | `min`, `max`, `value`, `step` |
...
| 수업 | 사용법 | 주요 매개변수 |
|---|---|---|
| `Container` | 범용 용기 | `children` |
| `Row` | 병렬 레이아웃 | `children`, `gap` |
| `Column` | 수직 레이아웃 | `children`, `gap` |
| `Tabs` | 탭 전환 | `tabs`(각각 `{label, content}`) |
| `Collapsible` | 축소 | `label`, `default_open`, `children` |
| `Card` | 머리글/본문/바닥글이 포함된 카드 | `header`, `body`, `footer` |

### stream.py (스트리밍 유형 2종)

| 수업 | 사용법 | 주요 매개변수 |
|---|---|---|
| `Stream` | 상태 기반 스트림 표시 | `states`(딕셔너리) |
| `Indicator` | 단일 상태 표시기 | `label`, `state`, `animation` |

### custom.py (커스텀 1종)

| 수업 | 사용법 | 주요 매개변수 |
|---|---|---|
| `Custom` | 정의되지 않은 위젯 | `custom_type`, `fallback`, `data` |
