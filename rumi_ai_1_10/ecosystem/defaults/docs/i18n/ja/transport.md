...
Defaults Pack は、フロントエンドおよびクライアントとの通信手段として 3 種類のトランスポートを提供します。どちらも `transport/` ディレクトリにあります。

---

## 概要

トランスポート層は、クライアントからのリクエストを受け取り、対応するブロックの`run(input_data, context)`関数を呼び出し、応答を返す中間層です。すべてのトランスポートは同じブロックを呼び出しますが、利用可能なルートの範囲が異なります。 HTTP トランスポートはほとんどのルートを提供し、stdio/UDS トランスポートはコア ルートのサブセットのみを提供します。
...
`_setup_routes()` メソッドを使用して、すべてのルートを `(method, pattern, handler)` タプルとして定義します。パターン内の `{param}` は正規表現 `(?P<param>[^/]+)` に変換され、コンパイルされた正規表現として保持されます。
...
stdio トランスポートは、HTTP トランスポートのサブセットを提供します。 `_ROUTE_MAP` または `transport/stdio.py` で定義されているすべてのルートは次のとおりです。
...
**合計: 16 ルート** (14 ハンドラー モジュール + 2 インライン システム)

### HTTP にのみ存在する (stdio には存在しない) ルート

次のルートは HTTP トランスポートでのみ使用でき、stdio トランスポートには含まれません。

- `POST /api/chat/conversations/{id}/summarize` — 要約/トリミング
...
- `POST /api/agent/{id}/instruct` — エージェントの指示を追加
- `POST /api/consent/check` — 同意チェック
- `POST /api/consent/{id}/confirm` — 同意の確認
- `PUT /api/prompts/{name}` — プロンプトアップデート
- `DELETE /api/prompts/{name}` — プロンプトが削除されました
- `POST /api/prompts/convert` — プロンプト ↔ ツール変換
- `POST /api/tools/create` — ツールの作成
- `PUT /api/tools/{name}` — ツールのアップデート
- `DELETE /api/tools/{name}` — ツールの削除
- `GET /api/tools/{name}/export` — ツールのエクスポート
- `GET /api/dev/inspect` — 開発: 検査
- `GET /api/dev/prompt-history` — 開発: プロンプト履歴
- `POST /api/dev/edit-prompt` — 開発: プロンプトの編集
- `POST /api/dev/replay` — 開発: リプレイ
- `GET /` — 静的ファイル (shell.html)
- `GET /static/{path}` — 静的ファイル配布
