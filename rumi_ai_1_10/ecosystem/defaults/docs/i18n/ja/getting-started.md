...
ブラウザで`http://127.0.0.1:8766/`にアクセスすると、`ui/shell.html`が返されます。 UIが表示されれば起動成功です。

HTTP サーバー ルート `/` は、`transport/http.py` の `_handle_static()` によって処理され、パック ルートに対する相対パス `ui/shell.html` を読み取って返します。追加の静的ファイル (CSS、JS、画像など) は `/static/{path}` でアクセスでき、`_handle_static_file()` は `ui/{path}` からファイルをロードします。たとえば、`/static/dev_panel.js` は `ui/dev_panel.js` を返します。
