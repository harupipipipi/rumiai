...
function checkAsset(asset, errors) {
  ...
  const packagedPath = path.join(UI_ROOT, asset.path);
  if (!existsSync(publicPath)) {
    errors.push(`missing webapp public asset: public/${asset.path}`);
    return;
  }
  if (!existsSync(packagedPath)) {
    errors.push(`missing packaged ui asset: ../ui/${asset.path}`);
    return;
  }
  ...
  if (packagedHash !== asset.sha256) {
    errors.push(`../ui/${asset.path} hash changed: expected ${asset.sha256}, got ${packagedHash}`);
  }
  if (publicHash !== packagedHash) {
    errors.push(`public/${asset.path} and ../ui/${asset.path} are out of sync`);
  }
}
...
console.log(`Vendored MediaPipe assets match ${MEDIAPIPE_TASKS_VISION.package}@${MEDIAPIPE_TASKS_VISION.version}`);
