        ...
        <form onSubmit={onSubmit} className="relative flex flex-col bg-[#2b2b2d] border border-zinc-700/30 rounded-[20px] overflow-visible shadow-2xl shadow-black/20 focus-within:border-zinc-500/60 transition-colors max-[640px]:rounded-2xl">
          ...
          <div className="flex items-center justify-between gap-2 px-4 pb-2 pt-0 max-[640px]:gap-1.5 max-[640px]:px-2 max-[640px]:pb-1.5">
            ...
            <div className="flex flex-shrink-0 items-center justify-end gap-2.5">
              <div className="flex items-center gap-2 bg-zinc-800/50 rounded-full px-3 py-1.5 border border-zinc-700/50 max-[640px]:hidden">
              ...
              <button type="submit" disabled={!input.trim() || isGenerating} title="送信" className="flex flex-shrink-0 items-center justify-center w-9 h-9 bg-zinc-200 text-black rounded-full disabled:opacity-30 disabled:cursor-not-allowed hover:bg-white transition-all shadow-sm max-[640px]:h-8 max-[640px]:w-8">
                {isGenerating ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} className="ml-1" />}
              </button>
            </div>
          </div>
        </form>
