...
export function ApprovalQueue({
  ...
  const renderApprovalRequest = (request: CodingApprovalRequest, active: boolean) => {
    ...
    const mcpReviewRows = mcpApprovalReviewRows(request);
    if (busy?.id === request.request_id) {
      viewModel.status = busy.decision === "approve" ? "approving" : "denying";
    }
    return (
      <div key={request.request_id}>
        <ApprovalDecisionSurface
          approval={viewModel}
          compact={!active}
          keyboardShortcuts={active ? { deny: "1", approve: "2" } : null}
        ...
        <p className="mt-1 px-1 text-[10px] text-zinc-600">
          {formatApprovalTime(request.created_at)} · {approvalStatusLabel(request, now)}
        </p>
      </div>
    );
  };
    ...
    <section className="border-b border-zinc-800/60 p-3" aria-label="Approval queue">
      <div className="mb-2 flex items-center justify-between gap-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className="rounded border border-zinc-800 px-1.5 py-0.5 text-[10px] text-zinc-500" title="Active pending approvals">{activeRequests.length}</span>
        </div>
        <button type="button" onClick={() => void load()} className="flex h-7 w-7 items-center justify-center rounded-md text-zinc-500 hover:bg-zinc-800 hover:text-zinc-100" title="Refresh approvals"><RefreshCw size={13} /></button>
      </div>
      ...
      <div className="space-y-2">
        ...
        {activeRequests.length === 0 && historyRequests.length > 0 && <p className="py-2 text-center text-[11px] text-zinc-600">No active approvals</p>}
        {historyRequests.length > 0 && <div className="space-y-2"><p className="px-1 text-[10px] uppercase tracking-wide text-zinc-600">Recent approval history</p>{historyRequests.map((request) => renderApprovalRequest(request, false))}</div>}
