...
type CloudflarePagesResult = {
  ...
  public_url?: string;
  project_name?: string;
  private_origin_warning?: boolean;
  error?: string;
  message?: string;
};

function CloudflarePagesSection({
  kernelBaseUrl,
  cloudflarePagesUrl,
  onSettingChange,
}: AppsSettingsPanelProps) {
  ...
  const [originUrl, setOriginUrl] = useState(defaultOrigin);
  const [projectName, setProjectName] = useState("");
  const [pagesUrl, setPagesUrl] = useState(cloudflarePagesUrl ?? "");
  const [busy, setBusy] = useState<"login" | "deploy" | null>(null);
  const [status, setStatus] = useState<string | null>(null);
  const [lastResult, setLastResult] = useState<CloudflarePagesResult | null>(null);
  const pagesQr = useQrDataUrl(pagesUrl.trim());

  useEffect(() => {
    setOriginUrl(defaultOrigin);
  }, [defaultOrigin]);
  ...
  const originIsPrivate = useMemo(() => {
    try {
      return isLoopbackHost(new URL(originUrl.trim()).hostname)
        || /^10\./.test(new URL(originUrl.trim()).hostname)
        || /^192\.168\./.test(new URL(originUrl.trim()).hostname)
        || /^172\.(1[6-9]|2\d|3[0-1])\./.test(new URL(originUrl.trim()).hostname);
    } catch {
      return false;
    }
  }, [originUrl]);

  const rememberPagesUrl = useCallback((value: string) => {
    const trimmed = value.trim();
    setPagesUrl(trimmed);
    onSettingChange?.("apps", "cloudflare_pages_url", trimmed);
  }, [onSettingChange]);
  ...
  const startLogin = useCallback(async () => {
    ...
    setStatus(null);
    try {
      const result = await mobileApiResources.createPublicUrl({
        provider_id: "cloudflare_pages_mobile",
        action: "login",
        local_url: originUrl.trim(),
      }) as CloudflarePagesResult;
      setLastResult(result);
      if (result.logged_in) {
        setStatus("Cloudflareは承認済みです。");
      } else if (result.login_running) {
        setStatus("Cloudflareの承認画面を開きました。承認後にPages URLを発行してください。");
      } else {
        setStatus(result.error ?? "Cloudflare承認を開始できませんでした。");
      }
    } catch (err) {
      setStatus(err instanceof Error ? err.message : "Cloudflare承認に失敗しました。");
    } finally {
      setBusy(null);
    }
  }, [originUrl]);

  const deployPages = useCallback(async () => {
    setBusy("deploy");
    setStatus(null);
    try {
      const result = await mobileApiResources.createPublicUrl({
      ...
      if (result.needs_login) {
        setStatus("Cloudflare承認が必要です。先に「Cloudflareで承認」を押してください。");
      } else if (result.ok && result.public_base_url) {
        rememberPagesUrl(result.public_base_url);
        setStatus("Cloudflare Pages URLを発行しました。");
      } else {
        setStatus(result.error ?? "Pages URLの発行に失敗しました。");
      }
    } catch (err) {
      setStatus(err instanceof Error ? err.message : "Pages URLの発行に失敗しました。");
    } finally {
      setBusy(null);
    }
  }, [originUrl, projectName, rememberPagesUrl]);
    ...
    <div className="space-y-4 rounded-lg border border-zinc-800 bg-zinc-950/40 p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <Cloud size={15} className="text-zinc-400" />
            <h4 className="text-sm font-medium text-zinc-100">Cloudflare Pagesで公開</h4>
          </div>
          <p className="mt-1 text-xs leading-5 text-zinc-500">
            Cloudflareにブラウザで承認し、固定のpages.dev URLを作成します。tokenのコピーは不要です。
          </p>
        </div>
        {pagesUrl && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-zinc-800 px-2.5 py-1.5 text-[11px] text-zinc-400 hover:border-zinc-600 hover:text-zinc-200"
          >
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <CopyField label="PC origin URL" value={originUrl} onChange={setOriginUrl} mono />
        <CopyField
      ...
      {originIsPrivate && (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-[11px] leading-5 text-amber-200">
          このoriginはローカル/LANアドレスです。Pages URL自体は固定化できますが、Cloudflare edgeからPCへproxyするには到達可能なorigin（named tunnelや公開HTTPS）が必要です。
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-700 px-3 py-2 text-xs text-zinc-200 hover:border-zinc-500 disabled:cursor-not-allowed disabled:opacity-50"
        >
          {busy === "login" ? <Loader2 size={13} className="animate-spin" /> : <Cloud size={13} />}
          Cloudflareで承認
        </button>
        <button
          type="button"
          disabled={busy !== null || !originUrl.trim()}
          onClick={() => void deployPages()}
          className="inline-flex items-center gap-1.5 rounded-lg bg-zinc-100 px-3 py-2 text-xs font-semibold text-zinc-950 hover:bg-white disabled:cursor-not-allowed disabled:opacity-50"
        >
          {busy === "deploy" ? <Loader2 size={13} className="animate-spin" /> : <Link2 size={13} />}
          Pages URLを発行
        </button>
      </div>

      {status && (
        <div className="rounded-lg border border-zinc-800 bg-black/30 px-3 py-2 text-[11px] leading-5 text-zinc-400">
          {status}
        </div>
      )}
      {lastResult?.private_origin_warning && (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-[11px] leading-5 text-amber-200">
          発行は完了しましたが、現在のoriginはCloudflareから直接到達できない可能性があります。リモート接続を使う場合はPC側に到達可能なoriginを設定してください。
        </div>
      )}
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_220px]">
        <CopyField
          label="Cloudflare Pages URL"
          value={pagesUrl}
          onChange={rememberPagesUrl}
          placeholder="https://rumi-mobile-xxxx.pages.dev"
          mono
        />
        <QrCard
          title="Pages QR"
          description="スマホで開く固定URL。"
          dataUrl={pagesQr.dataUrl}
          error={pagesQr.error}
          emptyHint="Pages URL発行後にQRが表示されます。"
        />
      </div>
    </div>
  );
...
export function AppsSettingsPanel({ kernelBaseUrl, cloudflarePagesUrl, onSettingChange }: AppsSettingsPanelProps) {
  return (
