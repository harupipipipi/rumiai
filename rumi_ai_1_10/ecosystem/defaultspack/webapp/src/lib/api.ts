...
export type P2PPairing = {
  pairing_id: string;
  code: string;
  status: "pending" | "claimed" | "approved" | "accepted" | "rejected" | "expired" | string;
  expires_at: number;
  created_at: number;
  peer_id?: string;
...
export type MobileDevicesResponse = { devices: MobileDevice[] };
...
export const api = {
  ...
  createCredentialTransfer(payload: {
    device_id: string;
    provider_id: string;
    api_id: string;
    provider_label?: string;
  ...
  confirmCredentialTransfer(
    transferId: string,
    payload: { device_id: string; provider_id: string; api_id: string; user_confirmed: true },
  ) {
