(function initRumiBridgeUrlPolicy(globalScope) {
  ...
  function validateNavigationUrl(value) {
    ...
    try {
      ...
      return { ok: false, message: "Navigation URL must be a valid HTTP(S) URL." };
    }

    if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
      return { ok: false, message: "Navigation URL must use HTTP or HTTPS." };
    }
    if (parsed.username || parsed.password) {
      return { ok: false, message: "Navigation URL must not include credentials." };
    }
    if (isLocalOrPrivateHost(parsed.hostname)) {
      return { ok: false, message: "Navigation URL must not use a local or private host." };
    }
  ...
  function normalizeNavigationUrl(value) {
    const result = validateNavigationUrl(value);
    return result.ok ? result.url : "";
  }

  function isTrustedSearchHomeOrigin(value) {
    let parsed;
    try {
