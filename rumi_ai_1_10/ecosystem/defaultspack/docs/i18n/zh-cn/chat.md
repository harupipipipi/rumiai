...
`standard_messages`。助手元数据记录了IR版本、模型路由、
聊天参考、规划警告、删除的功能和提供商跟踪信息。

回滚标志：

- `RUMI_DEFAULTSPACK_PROVIDER_LEGACY_MESSAGES=1`：强制继承
  标准消息提供者路径。
...
`AudiencePolicy`，选择`InputProfile`，然后调用`submit_input`。聊天
然后层接收附有外部元数据的普通用户消息。

外部对话应使用 `conversation_kind: "external"` 和 stable
会话密钥，例如 `slack:{team_id}:{channel_id}:{thread_id}` 或
`line:{source_type}:{source_id}`。答复应由
`ResponsePlanner`并由`ResponseAdapter`交付；聊天处理程序不应该
持有原始提供商令牌或直接构建提供商 API 调用。

## 创建一个对话

**处理程序**：`defaults.chat.create_conversation`（`blocks/chat/create_conversation.py`）**HTTP**：`POST /api/chat/conversations`**输入数据**：
...
**处理程序**：`defaults.chat.get_conversation`（`blocks/chat/get_conversation.py`）**HTTP**：`GET /api/chat/conversations/{id}`**输入数据**：

| Field | Type | Required | Description |
|---|---|---|---|
| `conversation_id` | `string` | Yes | Conversation ID (automatically injected from URL path) |

**返回值**： `ok(conv)` — 整个对话对象（包括消息）。如果未找到，则`error("Conversation not found", "NOT_FOUND")`。

## 对话列表

**处理程序**：`defaults.chat.list_conversations`（`blocks/chat/list_conversations.py`）**HTTP**：`GET /api/chat/conversations`**输入数据**：
...
**返回值**：`ok({"conversations": [...], "total": int})`。 `updated_at` 按降序排列。

## 更新对话

**处理程序**：`defaults.chat.update_conversation`（`blocks/chat/update_conversation.py`）**HTTP**：`PUT /api/chat/conversations/{id}`**输入数据**：
...
**处理程序**：`defaults.chat.delete_conversation`（`blocks/chat/delete_conversation.py`）**HTTP**：`DELETE /api/chat/conversations/{id}`**输入数据**：

| Field | Type | Required | Description |
|---|---|---|---|
| `conversation_id` | `string` | Yes | Conversation ID (automatically injected from URL path) |

**返回值**：`ok({"success": true})`。如果未找到，则`error("Conversation not found", "NOT_FOUND")`。

## 发送消息（有AI回复）

**处理程序**：`defaults.chat.send`（`blocks/chat/send.py`）**HTTP**：`POST /api/chat/conversations/{id}/messages`或`POST /v1/chat/completions`**输入数据**：
...
**处理流程**：在`ChatStore.add_message()`中保存用户消息→在`get_message_chain()`中获取对话历史→在`convert_to_standard()`中转换为标准格式→在`call_handler("defaults.ai.complete", ...)`中调用AI→在`build_assistant_message()`中构建助手消息→在`ChatStore.add_message()`中保存。**返回值**：`ok(assistant_msg)`——AI响应消息对象。
...
**处理程序**： `defaults.chat.add_message`（`blocks/chat/add_message.py`）**输入数据**：
...
**处理程序**： `defaults.chat.get_message`（`blocks/chat/get_message.py`）**输入数据**：
...
**处理程序**： `defaults.chat.update_message`（`blocks/chat/update_message.py`）**输入数据**：
...
**处理程序**： `defaults.chat.delete_message`（`blocks/chat/delete_message.py`）**输入数据**：
...
**处理程序**：`defaults.chat.stream`（`blocks/chat/stream.py`）**HTTP**：`POST /api/chat/conversations/{id}/stream`**输入数据**：
...
**处理**：存储用户消息并在`call_handler("defaults.ai.stream", ...)`中进行流式人工智能调用。 `stream_id`被返回，可用于停止流。**返回值**：`ok({"stream_id": "...", "conversation_id": "..."})`

## 停止直播

**处理程序**： `defaults.chat.stop`（`blocks/chat/stop.py`）**输入数据**：
...
**处理程序**： `defaults.chat.regenerate`（`blocks/chat/regenerate.py`）**输入数据**：
...
**处理**：删除指定消息 → 获取对话链直至父消息 → 再次发送给 AI → 保存新的助手消息。**返回值**： `ok(assistant_msg)` — 新的 AI 响应消息。

## 分支（对话分支）

**处理程序**：`defaults.chat.branch`（`blocks/chat/branch.py`）**HTTP**：直接 HTTP 路由未定义。通过 `call_handler("defaults.chat.branch", ...)`.**input_data** 调用：
...
**处理**：`ChatStore.branch()`通过将链复制到指定消息来创建新对话。新对话标题将附加 `" (branch)"`。消息中的`parent_id`/`children_ids`将被重新映射到新的ID。**返回值**：`ok(new_conv)`——新的分支对话对象。

## 搜索

**处理程序**： `defaults.chat.search`（`blocks/chat/search.py`）**输入数据**：
...
**处理**：`ChatStore.search()`对所有消息的`raw_text`字段执行不区分大小写的部分匹配搜索。**返回值**：`ok({"results": [msg, msg, ...]})`

## 导出

**处理程序**：`defaults.chat.export_conversation`（`blocks/chat/export_conversation.py`）**HTTP**：`POST /api/chat/conversations/{id}/export`**输入数据**：
...
**返回值**：`ok({"content": "..."})`。 `domain/chat/exporter.py`、`export_markdown()` 或 `export_json()` 被调用。

## 对话历史记录的人工智能摘要（summarize_and_trim）

**处理程序**：`defaults.chat.summarize_and_trim`（`blocks/chat/summarize_and_trim.py`）**HTTP**：`POST /api/chat/conversations/{id}/summarize`**输入数据**：
...
**处理**：获取指定范围内的消息→用`convert_to_standard()`转换为标准格式→构建摘要提示→让AI汇总→批量删除范围内的消息（`delete_messages_bulk`）→插入摘要消息（`insert_message_at`）。摘要消息`metadata`包括`is_summary: true`和`original_message_ids`。**返回值**：
...
**处理程序**：`defaults.chat.auto_trim`（`blocks/chat/auto_trim.py`）**HTTP**：`POST /api/chat/conversations/{id}/auto-trim`**输入数据**：
...
**处理**：获取对话的所有消息→从每条消息的内容中提取文本→向AI发送分析提示→AI以JSON数组形式返回可汇总的片段→通过检查消息ID是否存在进行验证。**返回值**：
...
实际的修剪可以通过将返回的`segments`的每个`start_id`/`end_id`传递到`summarize_and_trim`来执行。
