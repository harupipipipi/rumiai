...
`http://127.0.0.1:8766/`中独立UI的来源位于`webapp/`中。基于`dont_push_this_file/luxe-chat`，通过连接`defaultspack`的实际API进行管理。
...
此构建在`ui/`中输出`shell-app.js`和`shell-app.css`。 HTTP 服务器返回`ui/shell.html` 并从中读取`/static/shell-app.js` 和`/static/shell-app.css`。

## 发送第一次对话的步骤

### 在浏览器中打开

在浏览器中访问`http://127.0.0.1:8766/`会返回`ui/shell.html`。 `shell.html`是一个薄入口，仅安装`webapp`的建筑资产。如果出现界面，则说明启动成功。

HTTP 服务器根`/` 由`transport/http.py` 中的`_handle_static()` 处理，它读取并返回相对于Pack 根的路径`ui/shell.html`。其他静态文件（CSS、JS、图像等）可以在`/static/{path}`中访问，`_handle_static_file()`从`ui/{path}`加载文件。例如，`/static/shell-app.js` 返回`ui/shell-app.js`，`/static/dev_panel.js` 返回`ui/dev_panel.js`。
