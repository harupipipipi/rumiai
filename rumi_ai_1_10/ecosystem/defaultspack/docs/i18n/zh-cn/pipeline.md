...
**ParallelCaller** — 同时向多个模型发送请求并一起返回结果。使用`concurrent.futures.ThreadPoolExecutor`。**路由器** — 基于规则的输入分析并选择最佳模型或管道。**评估器** — 比较和评估多个响应以选择最佳的一个。将基本评分与法学硕士评委相结合。**管道** — 在多层中执行上述三者的组合。
