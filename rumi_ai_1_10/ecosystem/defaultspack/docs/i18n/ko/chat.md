...
`PreparedChatRun`은 이제 `chat_ir`, `ir_schema_version`를 전달합니다.
`provider_capabilities` 및 `provider_planning` 기존
`standard_messages`. 보조 메타데이터는 IR 버전, 모델 라우팅,
...
웹훅 및 게이트웨이 수신은 먼저 `ExternalEvent`을 생성하고 통과해야 합니다.
`AudiencePolicy`, `InputProfile`을 선택하고 `submit_input`로 전화하세요. 채팅
그러면 레이어는 외부 메타데이터가 첨부된 일반 사용자 메시지를 수신합니다.

외부 대화에서는 `conversation_kind: "external"`을 사용해야 하며 안정적이어야 합니다.
`slack:{team_id}:{channel_id}:{thread_id}`과 같은 세션 키 또는
`line:{source_type}:{source_id}`. 응답은 다음에 의해 계획되어야 합니다.
`ResponsePlanner` 및 `ResponseAdapter`에 의해 전달됨; 채팅 핸들러는 다음과 같은 행위를 해서는 안 됩니다.
