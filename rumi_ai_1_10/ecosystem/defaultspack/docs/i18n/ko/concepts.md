...
**`input_data`**은 요청 매개변수의 사전입니다. HTTP 요청의 본문은 JSON으로 구문 분석되고 URL 경로 매개변수(예: `conversation_id`)도 추가되어 전달됩니다.

**`context`**은 흐름 정보와 종속 함수를 포함하는 딕셔너리입니다. `transport/http.py`의 `_build_context()`은 다음 필드를 사용하여 컨텍스트를 구성합니다.

| 필드 | 유형 | 설명 |
|---|---|---|
| §루미§0§ | §루미§1§ | 흐름 ID. 직접 HTTP 호출을 위한 `"transport_direct"` |
| §루미§0§ | §루미§1§ | 단계 ID. 직접 HTTP 호출을 위한 `"http_request"` |
| §루미§0§ | §루미§1§ | 단계. §루미§2§ |
| §루미§0§ | §루미§1§ | ISO 8601 타임스탬프 |
| §루미§0§ | §루미§1§ | 발신자의 팩 ID입니다. §루미§2§ |
| §루미§0§ | §루미§1§ | 추가 입력 데이터 |
| §루미§0§ | §루미§1§ | 다른 핸들러를 호출하는 함수(커널을 통해 주입됨) |
...
- **`domain/company/message_router.py`** — `CompanySlackRuntime`: 채널/스레드/메시지/멘션/작업 기반 회사 라우팅.
- **`domain/agent/multi.py`** — 레거시 호환성만 해당됩니다.
- **`domain/tool/registry.py`** — `ToolRegistry`: 도구 정의 등록 및 관리. 하나씩 일어나는 것. 인메모리 + `user_data/shared/tools/`에 대한 지속성.
- **`domain/prompt/manager.py`** — `PromptManager`: 프롬프트 CRUD. 인메모리 + `user_data/shared/prompts/`에 대한 지속성.
- **`domain/prompt/template.py`** — `PromptTemplate`: 수동 프롬프트 템플릿 표현.
- **`domain/prompt/renderer.py`** — `render()`: `{{variable}}`를 템플릿 변수로 대체합니다.
...
- **HTTP** (`transport/http.py`): `DefaultsHttpServer`는 Python 표준 `http.server`를 사용하여 HTTP 서버를 시작합니다. URL 경로 및 메소드 라우팅, JSON 구문 분석, CORS 헤더 및 정적 파일 전달을 처리합니다.
- **stdio** (`transport/stdio.py`): 표준 입력/출력 전송입니다. CLI 및 파이프를 통한 통신에 사용됩니다.
- **UDS** (`transport/uds.py`): Unix 도메인 소켓 전송. 로컬 IPC에 사용됩니다.

## 위젯이란 무엇인가요?

위젯은 `lib/rumi_widgets/`에 정의된 UI 구성요소를 Python으로 표현한 것입니다. 위젯은 핸들러에서 프런트엔드로 전송되어 UI에 렌더링됩니다. 다음 모듈이 포함되어 있습니다:
...
핸들러는 `context["emit_widget"](§RUMI§0§)`을 사용하여 UI에 위젯을 보낼 수 있습니다.
...
| 필드 | 유형 | 설명 |
|---|---|---|
| §루미§0§ | §루미§1§ | 실행 중인 흐름 ID입니다. `"transport_direct"` 직접 통화 |
| §루미§0§ | §루미§1§ | 현재 단계 ID입니다. `"http_request"` 직접 통화 |
| §루미§0§ | §루미§1§ | 실행 단계. §루미§2§ |
| §루미§0§ | §루미§1§ | 타임스탬프(ISO 8601) |
| §루미§0§ | §루미§1§ | 발신자의 팩 ID |
| §루미§0§ | §루미§1§ | 추가 입력 데이터 |
| §루미§0§ | §루미§1§ | 다른 핸들러를 호출하는 함수 |
| §루미§0§ | §루미§1§ | 이벤트를 발생시키는 함수 |
| §루미§0§ | §루미§1§ | 이벤트를 기다리는 함수 |
| §루미§0§ | §루미§1§ | 위젯을 UI로 보내는 기능 |
| §루미§0§ | §루미§1§ | 취소 여부를 확인하는 기능 |
| §루미§0§ | §루미§1§ | 핸들러 설정(conditions.json 등) |
| §루미§0§ | §루미§1§ | 세션 정보(session_id, 작업공간 등) |

## InterfaceRegistry / EventBus와의 관계

**InterfaceRegistry**는 커널이 관리하는 인터페이스의 레지스트리입니다. 각 Pack에서 제공하는 인터페이스(핸들러)는 `call_handler`에 의해 이름 확인을 위해 등록되어 사용됩니다. `/api/context` 엔드포인트에서 `facade.list_interfaces()`을 호출하여 등록된 인터페이스 목록을 얻을 수 있습니다.

**EventBus**는 커널 관리 이벤트 버스입니다. `context["emit_event"](§RUMI§0§)`로 이벤트를 시작하고 `context["wait_event"](§RUMI§1§)`으로 이벤트를 기다릴 수 있습니다. 핸들러와 흐름 간의 비동기 통신에 사용됩니다.
