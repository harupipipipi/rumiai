...
호환성 엔드포인트. 내부에서는 `CompanySlackRuntime`에 회사 메시지를 게시하고 mentions/tasks/AgentEngine runs로 비동기적으로 라우팅합니다. 응답에는 `deprecation_warning`이 포함된다.
...
| `company_id` | 선택적 | `string` | 라우팅 대상 company workspace. 지정되지 않은 경우 default company |

**Response (`data`):**

| 필드 | 유형 | 설명 |
|---|---|---|
| `session_id` | `string` | 호환 session id. 실체는 company thread id |
| `status` | `string` | 라우팅 상태 |
| `turn_results` | `array` | 호환을 위한 빈 배열 |
| `result` | `object` | CompanySlackRuntime routing result |
| `deprecation_warning` | `string` | 호환 wrapper 공지 |

**오류 케이스:**

| 코드 | 설명 |
|---|---|
| `ERROR` |`task`이 지정되지 않았거나 company workspace routing 실패 |

---

### GET /api/agent/multi/{id}/status

호환 session id 에 대응하는 company thread 의 messages/tasks 를 취득한다. 경로 매개 변수 `{id}`가 `session_id`로 주입됩니다.
...
| 코드 | 설명 |
|---|---|
| `ERROR` |`session_id`이 지정되지 않음 |

---

### POST /api/agent/multi/{id}/message

호환 session thread에 메시지를 게시합니다. mention 은 active run 에의 runtime instruction, 또는 AgentEngine delegated task 로서 처리된다.
