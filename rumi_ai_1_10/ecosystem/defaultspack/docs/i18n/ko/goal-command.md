...
| 이름 | 유형 | 필수 | 기본값 | 설명 |
|------------------|---------|----------|---------|-----------------------------------------------------------------------------|
| `goal` | 문자열 | 예 | — | 작업자가 추구해야 하는 목표에 대한 자유로운 형식의 설명입니다.                 |
| `max_iterations` | 문자열 | 아니 | `5` | 최대 작업자/평가자 왕복. 1~20으로 고정됩니다.                      |
| `model` | 문자열 | 아니 | 활성 | 작업자 및 평가자 차례 모두에 전달되는 선택적 모델 힌트입니다.              |

슬래시 명령은 다음 위치에 등록됩니다.
[`commands/manifests/goal.json`](../commands/manifests/goal.json) 및
