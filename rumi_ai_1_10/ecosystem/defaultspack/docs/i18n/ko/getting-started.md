...
`http://127.0.0.1:8766/`의 독립형 UI 소스는 `webapp/`에 있습니다. `dont_push_this_file/luxe-chat`를 기반으로 `defaultspack`의 실제 API에 연결하여 관리한다.
...
프로덕션 버전에 해당하는 배포 파일을 업데이트하고 싶을 때 빌드합니다.
...
이 빌드는 `ui/`에서 `shell-app.js` 및 `shell-app.css`을 출력합니다. HTTP 서버는 `ui/shell.html`를 반환하고 여기에서 `/static/shell-app.js` 및 `/static/shell-app.css`를 읽습니다.

## 첫 번째 대화를 보내는 단계

### 브라우저에서 열기

브라우저에서 `http://127.0.0.1:8766/`에 액세스하면 `ui/shell.html`이 반환됩니다. `shell.html`는 `webapp`의 구축 자산을 장착하는 얇은 입구입니다. UI가 표시되면 시동에 성공한 것입니다.

HTTP 서버 루트 `/`는 `transport/http.py`의 `_handle_static()`에 의해 처리되며, 이는 Pack 루트에 상대적인 경로 `ui/shell.html`를 읽고 반환합니다. 추가 정적 파일(CSS, JS, 이미지 등)은 `/static/{path}`에서 액세스할 수 있으며 `_handle_static_file()`는 `ui/{path}`에서 파일을 로드합니다. 예를 들어, `/static/shell-app.js`은 `ui/shell-app.js`을 반환하고, `/static/dev_panel.js`는 `ui/dev_panel.js`을 반환합니다.
