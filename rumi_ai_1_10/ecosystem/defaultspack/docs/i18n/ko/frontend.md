...
현재 defaultspack 구현은 이 일반화에 추가됩니다. `PlacementManifest`
기본 배치 후보와 핀 지속성을 점차적으로 도입하고 있습니다. 첫 번째 응용 프로그램은
오른쪽 사이드바 주변 / 설정 / 작성기, 도구 관리자, 도구 필터 로그,
표면 인식 YOLO 토글, 모델 관리자, Webhook 끝점 등을 배포합니다.
나는 할 수 있다.
...
| 권한 | 설명 |
|------|------|
| `frontend.render.mount` | 도면 화면에 자산 배치 |
| `frontend.render.unmount` | 도면 표면에서 제거 |
| `frontend.render.update` | 도면 내용 업데이트 |
| `frontend.message.send` | 백엔드 → 그리기 표면 |
| `frontend.message.receive` | 그리기 표면 → 백엔드 |
| `frontend.message.stream` | 지속적으로 데이터 스트리밍 |
| `frontend.asset.register` | 자산 등록 수락 |
| `frontend.asset.unregister` | 자산 취소 |
| `frontend.asset.list` | 등록된 내용 |
| `frontend.layout.read` | 현재 레이아웃 정보 가져오기 |
| `frontend.layout.write` | 레이아웃 변경 및 저장 |
| `frontend.theme.read` | 현재 테마 정보 얻기 |
...
|----------|--------|------|
| `header` | 싱글 | 수평으로 정렬 |
| `sidebar.left` | 스택 | 수직으로 쌓으면 각 자산의 크기를 조정할 수 있습니다 |
| `sidebar.right` | 스택 | 수직으로 쌓다 |
| `main` | 탭 | 탭을 전환하고 분할할 수도 있습니다 |
| `panel.bottom` | 탭 | 탭 전환 |
| `statusbar` | 싱글 | 수평으로 정렬 |
| `floating` | 플로트 | 필요한 경우 오버레이 표시 |
...
|------|------|---------------|
| `text` | 텍스트 | 텍스트 |
| `code_block` | 코드 | 언어, 내용, 파일 이름, line_start |
| `diff` | 차이 | old_content, new_content, 파일 이름 |
| `image` | 이미지 | 원본, 대체, 너비, 높이 |
| `screenshot` | 스크린샷 | 소스, URL, 제목 |
| `progress` | 진행 | 라벨, 현재, 전체, 상태 |
| `terminal` | 터미널 출력 | 명령, 출력, 종료_코드 |
| `table` | 테이블 | 헤더, 행 |
| `chart` | 그래프 | 차트 유형, 데이터, 레이블 |
| `file_tree` | 파일 트리 | 나무 |
| `markdown` | 마크다운 | 내용 |
| `audio` | 오디오 | 소스, 기간 |
| `video` | 비디오 | 소스, 기간 |
...
| 유형 | 설명 | 주요 속성 |
|------|------|---------------|
| `input` | 텍스트 입력 | 자리 표시자, 값, 여러 줄 |
| `button` | 버튼 | 라벨, 액션, 변형 |
| `select` | 선택 | 옵션, 값, 다중 |
| `toggle` | 토글 | 라벨, 값 |
| `slider` | 슬라이더 | 최소, 최대, 값, 단계 |
| `checkbox` | 체크박스 | 라벨, 확인됨 |

**레이아웃 종류(6종)**

| 유형 | 설명 | 주요 속성 |
|------|------|---------------|
| `container` | 일반 컨테이너 | 어린이 |
| `row` | 나란히 | 어린이, 격차 |
| `column` | 수직 | 어린이, 격차 |
| `tabs` | 탭 전환 | 탭 |
| `collapsible` | 축소 | 라벨, default_open, 하위 |
| `card` | 카드 | 머리글, 본문, 바닥글 |
...
|------|------|---------------|
| `stream` | 상태 기반 스트림 | 상태 |
| `indicator` | 상태 표시기 | 라벨, 상태, 애니메이션 |

**커스텀(1종)**

| 유형 | 설명 | 주요 속성 |
|------|------|---------------|
| `custom` | 정의되지 않은 위젯 | custom_type, 대체, 데이터 |
