...
現在のdefaultspack実装は、この一般化に加えて`PlacementManifest`
拠点配置候補やピンの永続化も徐々に紹介していきます。最初のアプリケーションは、
右サイドバー/設定/コンポーザー、ツールマネージャー、ツールフィルターログ付近、
サーフェス対応の YOLO トグル、モデル マネージャー、Webhook エンドポイントなどをデプロイします。
できます。
