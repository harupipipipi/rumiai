...
[EN](../../defaultspack_extension_migration_plan.md) | [JP](./defaultspack_extension_migration_plan.md) | [KR](../ko/defaultspack_extension_migration_plan.md) | [CN](../zh-cn/defaultspack_extension_migration_plan.md)
<!-- docs-i18n-links:end -->
...
- 承認トークンは操作と引数のハッシュにバインドされます。
- ローカル アクションの試行と結果は、編集された JSONL 監査ログに書き込まれます。
- フロントエンド モデルのフォールバックとオプションの運用会社呼び出しはカタログです
  駆動される。
- `scripts/quality/scan_defaultspack_integrity.py --strict` ルート/ブロックをチェックします
  パリティ、フロントエンド/バックエンドルートパリティ、ローカルファーストデフォルト、機密ルート
  ガード配線、および新しい安全モジュールの構文。

残りの拡張機能作業はマニフェスト駆動のままにし、追加を避ける必要があります。
クラウドはデフォルトで新しいローカル ランタイムに戻ります。
