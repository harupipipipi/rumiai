...
`PreparedChatRun` に `chat_ir`、`ir_schema_version` が追加されました。
`provider_capabilities`、および既存の `provider_planning`
`standard_messages`。アシスタントのメタデータは、IR バージョン、モデル ルーティング、
チャットの参照、計画の警告、削除された機能、プロバイダーのトレース情報。

ロールバックフラグ:

- `RUMI_DEFAULTSPACK_PROVIDER_LEGACY_MESSAGES=1`: レガシーを強制する
...
`AudiencePolicy`、`InputProfile`を選択して、`submit_input`を呼び出します。チャット
