...
defaultspack チャット スタックは、内部表現として Rumi Chat IR v2 を使用します。
プロバイダーに依存しない ChatStore レコードとプロバイダー固有の API ペイロードの間。
ChatStore はプロバイダー固有のリクエストなしで Rumi メッセージを保存し続けます
状態。 IR 層は、メッセージ ID、親/子リンク、シーケンス番号、
メタデータ、マルチモーダル ブロック、ツール呼び出し/結果、推論ブロック、および不明
ブロック。

プロバイダーの実行は、次のような小さな段階に分割されます。

- `domain/chat/ir*.py`: Rumi Chat IR v2 データクラス、シリアル化、
  検証およびレガシーアダプター。
- `domain/ai_client/capabilities/`: プロバイダーの機能の明示と癖。
- `domain/ai_client/request_planner.py`: 劣化計画、警告、
  ドロップされた機能、ブリッジ アクション、およびツール名のエイリアス。
- `domain/ai_client/provider_compiler/`: OpenAI チャット、OpenAI 応答、
  OpenAI 互換、Google OpenAI、Google ネイティブ、Anthropic Messages、Bedrock
  Converse、およびローカルの OpenAI 互換ペイロード コンパイラ/パーサー。
- `domain/tool/protocol.py`: プロバイダーに依存しないツール定義、プロバイダー
  エイリアス、呼び出し、結果。
- `domain/chat/attachments/`: 添付ファイルのレコードとテキストの表現、
  インライン データ URL、画像ページ、PDF テキスト、トランスクリプト、プロバイダー ファイル ID。
- `domain/ai_client/provider_trace.py`: デバッグ用に編集されたトレース アーティファクト
  プロバイダーの計画とペイロード。

従来の StandardMessage パスは、次の場合を除き、デフォルトのランタイム パスのままです。
`RUMI_DEFAULTSPACK_PROVIDER_COMPILER_V2=1`が設定されています。設定
`RUMI_DEFAULTSPACK_PROVIDER_LEGACY_MESSAGES=1` は、たとえ
コンパイラフラグが有効になっています。
