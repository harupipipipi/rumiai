...
Compatibility endpoint. Internally, company messages are posted to `CompanySlackRuntime` and routed asynchronously as mentions/tasks/AgentEngine runs. The response includes `deprecation_warning`.
...
Get the messages/tasks of the company thread corresponding to the compatible session id. The path parameter `{id}` is injected as `session_id`.
...
**Response (`data`):** company thread status, messages, tasks, and compatibility warning.**Error case:**
...
Post a message to a compatible session thread. Mention is treated as a runtime instruction to active run or AgentEngine delegated task.
