...
- **Model Capability Routing** — Look at vision/tool/thinking/speed/knowledge_level and choose a real model within a model group. Pass image context to models that do not support images using Vision Bridge.
...
`webapp/` is a standalone frontend source that is based on `dont_push_this_file/luxe-chat` and connects to `/api/chat/...`, `/api/ui/...`, and `/api/health` of `defaultspack`. The output destination of `npm run build` is `ui/`, and the HTTP server delivers the built asset in `/` and `/static/...`.
