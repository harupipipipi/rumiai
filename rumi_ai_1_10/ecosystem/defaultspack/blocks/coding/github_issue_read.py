"""Read GitHub issue metadata and comments."""
...
OPERATION = "github.issue_read"
RISK = "medium"


def run(input_data, context=None):
    url = str(input_data.get("url") or input_data.get("issue_url") or "").strip()
    if not url:
    ...
    try:
        result = GitHubReadClient().issue(url)
        ...
        record_failure(OPERATION, RISK, str(exc), {"url": url})
        return error(str(exc), code="GITHUB_ISSUE_READ_ERROR")
