...
def _line_biz_prompt_prefix(response: dict[str, Any], *, mode: str = "") -> str:
    ...
    return (
        ...
        "Before typing, physically click inside the bottom reply composer/input field so the caret is there. "
        "Do not use Ctrl+A or select existing chat text. "
        "After typing, confirm the reply text is visible inside the bottom composer before sending. "
        "If transcript text becomes selected or the bottom composer still looks empty, click inside the bottom composer again and retry typing there; do not send an empty composer. "
