...
const JA_FIELD_COPY: Record<string, LocalizedFieldCopy> = {
  ...
  "deepthink.model_source": { label: "DeepThinkで使うモデル", help: "会話中のモデルを使うか、DeepThink専用モデルを選びます。" },
  "deepthink.model": { label: "DeepThink専用モデル", help: "特定providerには固定されません。" },
  "deepthink.allow_delegated_agents": { label: "DelegationへのDeepThink付与を許可", help: "Delegationが明示的に要求した場合だけ有効です。親タスクからは自動継承しません。" },
  "deepthink.allow_background_continuations": { label: "バックグラウンド継続でも許可", help: "CI完了後やmonitor・automationの継続でもDeepThinkを再実行します。通常はOFFを推奨します。" },
