    ...
    <div className={`group/file relative flex-shrink-0 overflow-hidden rounded-xl border border-white/[0.1] bg-[#1b1c20] shadow-sm ${isImage ? "h-24 w-24" : "h-[66px] w-[190px]"}`}>
      ...
      {attachmentNeedsSecurityReview(file) && onReview && (
        <button
          ...
          className="absolute bottom-2 right-2 inline-flex h-8 items-center gap-1 rounded-md border border-amber-500/40 bg-zinc-950/90 px-2 text-[10px] text-amber-200 focus:outline focus:outline-2 focus:outline-offset-2 focus:outline-amber-300"
...
export function AttachmentSecurityReviewDialog({
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      dialog?.querySelector<HTMLElement>("[data-attachment-review-cancel]")?.focus();
    });
    return () => {
  ...
  return (
    <div className="fixed inset-0 rumi-layer-modal flex items-center justify-center bg-black/70 p-4" role="presentation">
      <section
        ...
        aria-describedby="attachment-security-review-summary"
        onKeyDown={(event) => {
          if (event.key === "Escape") {
      ...
      >
        ...
        <dl className="mt-4 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 rounded-lg bg-zinc-900/70 p-3 text-xs">
          ...
          <dt className="text-zinc-500">Source</dt><dd>{file.source === "workspace" ? "Workspace API (trusted root; symlink escapes blocked)" : "Local file picker/drop"}</dd>
        </dl>
        ...
        <p id="attachment-security-review-summary" className="mt-3 text-xs leading-5 text-zinc-400">
        ...
        <div className="mt-5 flex flex-wrap justify-end gap-2">
          <button type="button" data-attachment-review-cancel onClick={onClose} className="rounded-lg px-3 py-2 text-sm text-zinc-300 hover:bg-zinc-900">Cancel</button>
            ...
            <div className="rumi-composer-attachment-strip mx-3 mt-2 flex max-w-[calc(100%-1.5rem)] gap-2 overflow-x-auto pb-1 max-[640px]:mx-2 max-[640px]:max-w-[calc(100%-1rem)]">
              ...
              {attachedFiles.map((file) => (
                <FilePreviewCard
                  ...
                  file={file}
                  onRemove={onFileRemove}
                  onReview={() => setAttachmentReviewId(file.id)}
                />
              ))}
            </div>
          )}
