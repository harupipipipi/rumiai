          ...
          <div className="mt-3 rounded-xl border border-zinc-800 bg-zinc-900/55 p-2.5" data-testid="calendar-time-contract">
            ...
            <div className="mt-2 grid grid-cols-2 gap-1.5" role="group" aria-label="時刻の保持方法">
              <button
                ...
                className={cn(
                  "min-h-11 rounded-lg border px-2 py-1 text-left text-[11px]",
              ...
              >
                ...
                <span className="block text-[10px] opacity-70">移動後も{normalizedDraftTime}</span>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <span className="block text-[10px] opacity-70">ゾーン変更でもUTCを維持</span>
              </button>
            </div>
            ...
            {draftTemporalResolution?.status === "nonexistent" && (
              <div className="mt-2 rounded-lg border border-red-400/40 bg-red-500/10 p-2 text-[11px] text-red-100">
                <p>{draftTemporalResolution.message}</p>
            ...
            {draftTemporalResolution?.status === "invalid" && (
              <p className="mt-2 rounded-lg border border-red-400/40 bg-red-500/10 p-2 text-[11px] text-red-100">
                {draftTemporalResolution.message}
              </p>
            )}
            {resolvedDraftLabels && (
              <div className="mt-2 text-[11px] text-zinc-300" role="status" aria-live="polite" data-testid="calendar-resolved-time">
                <p className="font-medium">実行予定: {resolvedDraftLabels.local}</p>
                <p className="mt-0.5 font-mono text-[10px] text-zinc-500">{resolvedDraftLabels.utc}</p>
              </div>
            )}
            {storedInstantChanged && (
              <div className="mt-2 rounded-lg border border-blue-400/30 bg-blue-500/10 p-2 text-[10px] text-blue-100">
            ...
            {activeEditor.startKey !== activeEditor.endKey && (
              <p className="mt-2 text-[10px] text-zinc-500">複数日の項目では、Agentの実行時刻は開始日のみに適用されます。</p>
            )}
          </div>
          {draftKind === "task" && (
