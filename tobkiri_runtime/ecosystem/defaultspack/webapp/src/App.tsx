...
function CalendarComposerPanel({
  ...
  useEffect(() => {
    ...
    void api.listSchedules().then((response) => {
      ...
      if (reconciliation.issues.length > 0) setCalendarReconciliation(reconciliation);
    }).catch((error) => {
      if (!cancelled) {
  ...
  useEffect(() => {
    const handleStorage = (event: StorageEvent) => {
      ...
      if (next.document.revision > calendarLoad.document.revision || next.status === "corrupt") {
  ...
  const submitDraft = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const proposedItems = activeEditor.mode === "edit"
        ...
        : [...items, nextItem];
      const mutation = beginDurableMutation("upsert", proposedItems);
  ...
  const deleteActiveItem = async () => {
    ...
    setDraftError(null);
    try {
      const mutation = beginDurableMutation("delete", items.filter((item) => item.id !== activeItem.id));
  ...
  const runActiveAgentNow = async () => {
    ...
    try {
      ...
      const mutation = beginDurableMutation("upsert", items.map((item) => item.id === activeItem.id ? { ...item, lastRunStatus: status } : item));
      applySettledMutation(mutation);
  ...
  const repairCalendarState = async () => {
    try {
      ...
      const data = isRecord(response.data) ? response.data : response;
      const schedules = Array.isArray(data.schedules) ? data.schedules : [];
      const pendingItems = calendarLoad.pendingMutation?.proposedItems ?? calendarReconciliation?.items ?? calendarLoad.document.items;
      const repairItems = reconcileCalendarSchedules(pendingItems, schedules).items;
      const document = repairCalendarDocument(window.localStorage, calendarWriterIdRef.current, repairItems);
      setCalendarLoad({ document, pendingMutation: null, recoveryRaw: calendarLoad.recoveryRaw, status: "ready" });
      setCalendarReconciliation(null);
      setDraftError(null);
    } catch (error) {
      setDraftError(error instanceof Error ? error.message : "Calendar repair failed.");
    }
  };

  const cancelPendingCalendarChange = () => {
    try {
      setCalendarLoad(cancelCalendarMutation(window.localStorage, calendarWriterIdRef.current));
      setDraftError(null);
    } catch (error) {
      setDraftError(error instanceof Error ? error.message : "Pending Calendar change could not be cancelled.");
    }
  };
          ...
          <span>
            {calendarLoad.status === "corrupt"
              ...
              : calendarLoad.pendingMutation
                ...
                : `${calendarReconciliation?.issues.length ?? 0} Calendar sync issue(s) need review.`}
          </span>
          <span className="flex flex-shrink-0 gap-2">
            {calendarLoad.pendingMutation && <button type="button" className="rounded border border-amber-500/60 px-2 py-1 hover:bg-amber-900/60" onClick={cancelPendingCalendarChange}>Cancel</button>}
            <a
