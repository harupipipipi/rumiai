...
def handle(envelope: RumiInputEnvelope, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    runtime_context = _delegate_context(envelope, context or {})
    model_service = ModelRuntimeSettingsService()
    model_settings = model_service.get_settings()
    legacy_model = str(
        payload.get("model") or payload.get("profile_id") or payload.get("preferred_model") or ""
    ).strip()
    delegate_params = dict(payload.get("params") if isinstance(payload.get("params"), dict) else {})
    ...
    conversation_id = str(runtime_context.get("conversation_id") or "").strip()
    ...
    try:
        model_policy_receipt = resolve_secondary_model_policy(
            payload.get("model_policy") or delegate_params.get("model_policy"),
            payload.get("thinking_policy") or delegate_params.get("thinking_policy"),
            context=policy_context,
            profiles=model_service.list_profile_catalog(),
            legacy_model=legacy_model,
            legacy_thinking_level=str(
                payload.get("thinking_level") or delegate_params.get("thinking_level") or ""
            ),
            required_capabilities=required_capabilities,
            replay_receipt=delegate_params.get("model_policy_receipt")
            if isinstance(delegate_params.get("model_policy_receipt"), dict)
            else None,
        )
    ...
    result = execute_agent(
        {
            "task": task,
            "tools": list(
                payload.get("tools") if isinstance(payload.get("tools"), list) else envelope.tools
            ),
            "model": str(model_policy_receipt["resolved_profile_id"]),
            ...
            "model_policy": model_policy_receipt["requested_model_policy"],
            "thinking_policy": model_policy_receipt["requested_thinking_policy"],
            "thinking_level": model_policy_receipt["thinking_level"],
            "model_policy_receipt": model_policy_receipt,
            "attachments": list(
                payload.get("attachments")
                if isinstance(payload.get("attachments"), list)
                else envelope.attachments
            ),
    ...
    if isinstance(result, dict) and result.get("status") == "ok":
        ...
        delegate["model_policy_receipt"] = model_policy_receipt
...
def _delegate_payload(envelope: RumiInputEnvelope) -> dict[str, Any]:
    payload = (
        envelope.params.get("delegate") if isinstance(envelope.params.get("delegate"), dict) else {}
    )
    if payload:
        return dict(payload)
...
def _delegate_summary(
    data: dict[str, Any], payload: dict[str, Any], envelope: RumiInputEnvelope
) -> dict[str, Any]:
    return {
...
def _safe_authority_approval_result(
    data: dict[str, Any], approval: dict[str, Any], assistant_text: str
) -> dict[str, Any]:
    safe = {
        "status": "authority_approval_required",
        "code": "authority_approval_required",
...
def _safe_failed_delegate_result(
    data: dict[str, Any], assistant_text: str, code: str
) -> dict[str, Any]:
    safe: dict[str, Any] = {
