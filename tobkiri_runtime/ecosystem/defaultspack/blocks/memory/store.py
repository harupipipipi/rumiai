"""defaults.memory.store — メモリへの保存ハンドラー.
...
戻り値:
    {"status": "ok", "data": {"id": str, "stored": true}}
"""
...
def run(input_data: Dict[str, Any], context: Any = None) -> Dict[str, Any]:
    ...
    try:
        ...
        store = MemoryStore()
        entry = store.store(content=content, metadata=metadata)

        return ok({"id": entry["id"], "stored": True})

    except Exception as exc:
        return error(f"Failed to store memory: {exc}", "STORE_ERROR")
