"""defaults.memory.vector_query — ベクトルストアからの検索ハンドラー.
...
def run(input_data: Dict[str, Any], context: Any = None) -> Dict[str, Any]:
    ...
    try:
        ...
        store = MemoryStore()
        results = store.vector_search(query=query, limit=limit)

        return ok({"results": results})

    except Exception as exc:
        return error(f"Failed to query vector store: {exc}", "VECTOR_QUERY_ERROR")
