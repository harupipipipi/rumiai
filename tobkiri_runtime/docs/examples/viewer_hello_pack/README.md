...
Tobkiri の **viewer:display** capability を使うサンプル Pack です。
Tobkiri Viewer 内に Hello World フロントエンドを表示します。
...
`viewer:display` は Tobkiri の core capability の一つで、Pack が Tobkiri Viewer（Tauri ベースのデスクトップ UI）にフロントエンドを表示するための権限です。
...
Tobkiri Viewer を起動すると、承認・Grant 済みの Pack のフロントエンドが表示されます。
