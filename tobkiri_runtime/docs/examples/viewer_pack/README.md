...
`viewer:display` capability を使って Tobkiri Viewer にフロントエンドを表示する Pack の最小限の例です。
...
`web_mount` フィールドにより、`web/` ディレクトリの内容が Tobkiri Viewer 内で配信されます。
...
Pack のフロントエンドです。Tobkiri Viewer の sandbox WebView 内にロードされます。
...
6. Tobkiri Viewer が `web_mount_url` を sandbox WebView にロード
