...
def test_opencode_zen_mimo_free_stream_uses_completion_and_synthetic_events(
    monkeypatch,
):
    provider = _provider(monkeypatch)
    captured = {}
...
def test_opencode_zen_mimo_free_empty_completion_is_recoverable_error(
    ...
    with patch.object(provider, "_request_openai_json", return_value=raw):
