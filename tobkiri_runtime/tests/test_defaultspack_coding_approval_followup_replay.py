...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "page.click",
            {"action": "page.click", "client_id": "client-1", "element_id": "el-7"},
...
def test_browser_companion_followup_replays_authoritative_stored_arguments(
    ...
    """Issue #857: approval resumes the server-stored Browser Companion call.

    The UI payload is display-only and may be empty (the reported ``session``
    case) or stale.  Replay must re-read the approved request and execute its
    exact stored arguments once, including ``page.*`` operations.
    """
    from domain.chat.run_request import PreparedChatRun
    from domain.chat.stream_engine import ChatRunEngine
    from domain.safety import approval
