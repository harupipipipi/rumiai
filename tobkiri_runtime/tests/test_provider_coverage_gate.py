...
def test_real_matrix_report_is_deterministic_and_required(tmp_path):
    first = build_report()
    second = build_report()
    assert first == second
    assert first["provider_count_expected"] == 82
    assert first["provider_count_registered"] == 82
    assert first["passed"] is True
    assert first["failure_count"] == 0
    assert all(not values for values in first["failures"].values())
