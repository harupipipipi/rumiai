...
LOCAL_DEV_TAURI_CONFIG = TAURI_ROOT / "tauri.local-dev.conf.json"
...
PANEL_SESSION_CAPABILITY = TAURI_ROOT / "capabilities" / "panel-session.json"
LAUNCHER_PANEL_CAPABILITY = TAURI_ROOT / "capabilities" / "launcher-panel.json"
PLATFORM_TARGETS = {
    "windows": ["nsis"],
    "macos": ["dmg"],
    "linux": ["deb", "appimage"],
...
def test_tauri_command_manifest_matches_invoke_handler_and_generated_acl():
    """Keep the app command inventory and generated ACL permissions in sync."""
    command_sources = "\n".join(
        path.read_text(encoding="utf-8")
        for path in sorted((TAURI_ROOT / "src").glob("*.rs"))
    )
    handlers = set(
        re.findall(
    ...
    build_source = BUILD_RS.read_text(encoding="utf-8")
    manifest_block = build_source.split(
        "AppManifest::new().commands(&[", 1
    )[1].split("]),", 1)[0]
    manifest_handlers = set(re.findall(r'"([A-Za-z0-9_]+)"', manifest_block))

    invoke_block = LAUNCHER_RUNTIME.read_text(encoding="utf-8").split(
        "tauri::generate_handler![", 1
    )[1].split("])\n", 1)[0]
    invoke_handlers = set(
        re.findall(r"(?:[A-Za-z0-9_]+::)?([A-Za-z0-9_]+)", invoke_block)
    )

    assert manifest_handlers == handlers
    assert invoke_handlers == handlers
    for handler in handlers:
        assert (
            TAURI_ROOT / "permissions" / "autogenerated" / f"{handler}.toml"
        ).is_file()


def test_launcher_panel_capabilities_are_split_from_session_renewal():
    session = _read_json(PANEL_SESSION_CAPABILITY)
    launcher_panel = _read_json(LAUNCHER_PANEL_CAPABILITY)

    assert session["permissions"] == ["allow-reauthorize-panel-session"]
    assert launcher_panel["permissions"] == [
        "allow-get-background-control-status",
        "allow-get-desktop-system-info",
        "allow-launch-defaultspack-desktop",
        "allow-open-external-url",
        "allow-send-to-background",
        "allow-show-app-window",
    ]
    assert session["remote"] == launcher_panel["remote"]


def test_local_dev_tauri_config_disables_production_preparation_hooks():
    config = _read_json(LOCAL_DEV_TAURI_CONFIG)
    build_script = BUILD_AND_SIGN.read_text(encoding="utf-8")

    assert config["build"]["beforeBuildCommand"] is None
    assert config["build"]["beforeDevCommand"] is None
    local_branch = build_script.split(
        'if [[ "$mode" == "local-dev" ]]; then', 1
    )[1].split("fi\n\n", 1)[0]
    assert "--config src-tauri/tauri.local-dev.conf.json" in local_branch
    assert "--config src-tauri/tauri.conf.json" not in local_branch
...
def test_unsigned_shell_exception_is_bound_to_exact_generated_development_artifacts():
    ...
    assert "return bundled_artifacts_match or (" in source
