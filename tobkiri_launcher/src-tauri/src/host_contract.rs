...
pub(crate) fn write_contract(
    ...
    fs::create_dir_all(parent)
        .with_context(|| format!("failed to create host contract directory {}", parent.display()))?;
    restrict_owner_only(parent)?;

    let mut merged_values: BTreeMap<String, String> = fs::read(&path)
        .ok()
    ...
    payload.insert("schema_version".into(), Value::String("tobkiri.host-contract.v1".into()));
    payload.insert(
        "profile_id".into(),
        Value::String(profile_id.trim().to_string()),
    );
    payload.insert(
        "values".into(),
